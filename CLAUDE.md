...
`.onInit` do NSIS define `$IsUpgrade` detectando somente `state\install-state.json` — nunca `app\`/`config\` isolados, pois esses resíduos sobrevivem ao rollback de uma instalação nova que falhou (`Uninstall-PowerShellConfig.ps1 -RollbackOnly` sempre apaga `install-state.json`, mas não toca `app/`/`config/`; a remoção usa `try/catch` explícito, não `-ErrorAction SilentlyContinue` — uma falha ao apagar `install-state.json` propaga erro em vez de deixar instalações futuras presas em modo upgrade para sempre); tratar esse resíduo como upgrade pularia toda a configuração de Terminal/perfil/cor numa reinstalação. Em upgrade real: extrai o novo app em `app.update`, troca por `Rename` (`app` → `app.previous` → swap, com rollback). Fontes, bloco do `$PROFILE`, settings/fragmento do Windows Terminal, terminal padrão e sincronização de startup (`--sync-startup`) são **sempre reverificados/reparados**, mesmo em upgrade — são idempotentes e baratos (`Sync-ManagedFile` por hash, `Set-ManagedProfileBlock`/`Merge-TerminalSettings` por edição cirúrgica). Só o que é caro e não relacionado à integração gerenciada continua pulado em upgrade: preflight/instalação de pacotes WinGet e reinstalação dos módulos PowerShell (`posh-git`, `Terminal-Icons`, `PSReadLine`), além da `executionPolicy` (não reforça política que o usuário pode ter alterado deliberadamente). `Install-PowerShellConfig.ps1` recebe `-IsUpgrade` explicitamente do `.nsi` (que já sabe com certeza); `Configure-PowerShellConfig.ps1` **não** recebe esse parâmetro — mantém a autodetecção própria via `install-state.json` (mesma fonte de verdade, lida no timing certo). O preflight de "Windows Terminal fechado" em `Install-PowerShellConfig.ps1` vira aviso não bloqueante em upgrade (a escrita já é atômica, tmp+rename); o preflight equivalente em `.onInit` (`EnsureWindowsTerminalClosed`) continua bloqueante e pulado em upgrade. `config/`, `state/` e `backups/` nunca são removidos por upgrade. Falha de configuração em upgrade não dispara rollback integral (`Configure-PowerShellConfig.ps1`) — só loga e preserva o estado gerenciado anterior. Rollback de instalação nova (`-RollbackOnly`) remove `config/settings.json` e `config/themes/active.omp.json` se ainda intocados desde a instalação (`ManagedConfig` no estado + `Remove-ManagedFileIfPristine` em `Common.ps1`), para não deixar o app com um `colorScheme` órfão.
